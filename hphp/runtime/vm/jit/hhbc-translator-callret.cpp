...
// rrh was here {
void HhbcTranslator::emitRetProfilerHook(SSATmp* fp, SSATmp* retVal,
                                         Block* catchBlock,
                                         bool suspendingResumed) {
  //
  // consider gdb breakpoint on
  //   HPHP::JIT::X64::CodeGenerator::cgHotProfilerFunctionReturnHook
  //
  static bool hotProfilerEnabled = true;
  if (hotProfilerEnabled) {
    if (retVal != nullptr) {
      /*
       * TODO: if this is set to null,
       * get asserts involving IR instruction invariants
       */
      // catchBlock = 0; 
      gen(HotProfilerFunctionReturnHook, RetCtrlData(false),
         catchBlock, fp, retVal);
    } else {
      // Is this path for generators?
      // We don't bother with profiling anything like SuspendHook
    }
  }
}
...
void HhbcTranslator::emitRet(Type type) {
  ...
  // rrh was here {
  static int where_mask = 0x0;  // bit 0 ==> early, bit 1 ==> late
  if (where_mask & (1<<0)) {
    // This is like emitRetSurpriseCheck, but before decref and gc kicks in
    emitRetProfilerHook(m_irb->fp(), retVal, catchBlock, false);
  }
  if (where_mask & (1<<1)) {
    emitRetProfilerHook(m_irb->fp(), retVal, catchBlock, false);
  }
